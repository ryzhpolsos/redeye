...
pIsTopLevelWindow fIsTopLevelWindow = NULL;

BOOL IsTopLevelWindow(HWND hWnd){
    if(!fIsTopLevelWindow(hWnd)) return FALSE;

    WCHAR szWndClass[7];
    GetClassName(hWnd, szWndClass, 7);

    if(szWndClass[0] == '#'){
        // 32770 - dialog box class
        return wcscmp(szWndClass, TEXT("#32770")) == 0;
    }

    LONG_PTR wndStyle = GetWindowLongPtr(hWnd, GWL_STYLE);
    LONG_PTR wndExStyle = GetWindowLongPtr(hWnd, GWL_EXSTYLE);

    return (wndStyle & WS_CAPTION) && (wndExStyle & WS_EX_OVERLAPPEDWINDOW); 
}
...
BOOL WINAPI DllMain(HINSTANCE hInst, DWORD fdwReason, LPVOID lpvReserved){
    switch(fdwReason){
        case DLL_PROCESS_ATTACH: {
            ...
            GetWindowThreadProcessId(rwMsgWin, &rwProcessId);

            HMODULE user32 = GetModuleHandle(TEXT("user32.dll"));
            if(user32){
                fGetDpiForWindow = (pGetDpiForWindow)GetProcAddress(user32, "GetDpiForWindow");
                fIsTopLevelWindow = (pIsTopLevelWindow)GetProcAddress(user32, "IsTopLevelWindow");
            }
...
LRESULT WINAPI HookWndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam){
    ...
    if(IsTopLevelWindow(hWnd)){
        switch(uMsg){
            case WM_CREATE: {
                if(pSharedData->shellHook || wpe.hWrapper){
            ...
            case WM_PAINT: {
                ...
                if(pid != rwProcessId && !wpe.hWrapper){
                    RECT rc;
                    GetWindowRect(hWnd, &rc);

                    HWND hWrapper = (HWND)SendMessage(rwMsgWin, wmxResMsg, WMX_RES_WRAPPERREQUEST, (LPARAM)hWnd);
                    wpe.hWrapper = hWrapper;

                    SetWindowLongPtr(hWnd, GWL_STYLE, WS_VISIBLE);
                    SetWindowLongPtr(hWnd, GWL_EXSTYLE, WS_EX_TOOLWINDOW);
                    SetParent(hWnd, hWrapper);
                }
                #endif
